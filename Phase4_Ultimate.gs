...
function saveResults_Ultimate(ss, allData, byClass, headersRef) {
  ...
  for (const className in byClass) {
    ...
    if (!sheet) {
      logLine('WARN', `⚠️ Onglet ${className} introuvable pour l'écriture.`);
      errorCount++;
      continue;
    }
    ...
    try {
      ...
      if (rowsToWrite.length > 0) {
        ...
        logLine('INFO', `  ✅ ${className} : ${indices.length} élèves écrits.`);
        successCount++;
      }
    } catch (e) {
      logLine('ERROR', `  ❌ Erreur écriture ${className} : ${e.toString()}`);
      errorCount++;
    }
  }
